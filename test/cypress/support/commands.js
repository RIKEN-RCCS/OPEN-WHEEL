...
Cypress.Commands.add("loginPageDisplayed", ()=>{
  cy.get("img").first()
    .should("be.visible")
    .and("have.attr", "alt", "wheel title logo");
  cy.contains("Login");
  cy.get("[data-cy=\"username\"]").should("be.visible");
  cy.get("[data-cy=\"password\"]").should("be.visible");
  cy.get("[data-cy=\"submit\"]").should("be.visible");
});
Cypress.Commands.add("homePageDisplayed", ()=>{
  cy.get("img").first()
    .should("be.visible")
    .and("have.attr", "alt", "wheel title logo");
  cy.contains("home");
});
Cypress.Commands.add("remotehostPageDisplayed", ()=>{
  cy.get("img").first()
    .should("be.visible")
    .and("have.attr", "alt", "wheel title logo");
  cy.contains("remotehost");
});
Cypress.Commands.add("workflowPageDisplayed", ()=>{
  cy.get("img").first()
    .should("be.visible")
    .and("have.attr", "alt", "wheel title logo");
...
Cypress.Commands.add("assertAll", ()=>{
  ...
  if (Object.keys(errors).length > 0) {
    const errorMessages = Object.entries(errors).map(([title, entries], index)=>{
      const errorMessage = (entries).map(({ error })=>{return (
        `${"=> "+error.message}`
      )}).join("\n\n")
...
Cypress.Commands.add("projectOpen", (projectName)=>{
  cy.contains("tr", projectName, { timeout: 1000000 }).find("[type=\"checkbox\"]")
    .click({force: true})
...
Cypress.Commands.add("removeTask", (taskName)=>{
  cy.contains("header", "name").find("button")
    .eq(2)
    .click()
    .then(()=>{
      cy.get("svg").contains(taskName)
...
Cypress.Commands.add("closeTask", ()=>{
  cy.get(".v-toolbar-items").find("[type=\"button\"]")
    .eq(0)
    .click()
})
...
Cypress.Commands.add("clickInputOutputFilesTab", ()=>{
  cy.contains("input/output files").click()
...
Cypress.Commands.add("renameInputFiles", (fileName, fixName)=>{
  cy.contains("input/output files").next()
    .contains("div", "input files")
    .contains("button", fileName)
    .click()
  cy.get(".v-overlay__content").find("input")
    .clear()
    .type(fixName, {enter: true})
})
...
Cypress.Commands.add("selectHost", (hostName)=>{
  cy.contains("[role=\"listbox\"]", hostName).contains(hostName)
    .click()
})
...
Cypress.Commands.add("openRetrySettingTab", ()=>{
  cy.contains("retry setting").click()
    .wait(animationWaitTime)
})
...
Cypress.Commands.add("addIncludeFile", (fileName)=>{
  cy.contains("table", "include").parent()
    .siblings()
    .find("input")
    .type(fileName)
  cy.contains("table", "include").parent()
...
Cypress.Commands.add("openScriptSelectBox", ()=>{
  cy.contains("label", /^script$/).parent()
    .click()
})
...
Cypress.Commands.add("clickConsole", ()=>{
  cy.get("footer").find("[type=\"button\"]")
    .eq(0)
    .click()
})
...
Cypress.Commands.add("switchUseJobScheduler", (flg)=>{
  if (flg === "on") {
    cy.contains("label", "use job scheduler").siblings()
      ...
      .then(($el)=>{
        if (!$el) {
          cy.contains("label", "use job scheduler").click({force: true})
            .wait(animationWaitTime)
        }
      })
  } else if (flg === "off") {
    cy.contains("use job scheduler").siblings()
      ...
      .then(($el)=>{
        if ($el) {
          cy.contains("use job scheduler").click({force: true})
            .wait(animationWaitTime)
        }
      })
  }
...
Cypress.Commands.add("retryNumberType", (retryNum)=>{
  ...
  cy.contains("number of retry").siblings()
    .type(retryNum, {enter: true})
})
...
Cypress.Commands.add("scriptNameForConditionCheckSelect", (scriptName)=>{
  cy.contains("label", "script name for condition check").parent()
    .click()
  cy.selectListBox(scriptName)
})
