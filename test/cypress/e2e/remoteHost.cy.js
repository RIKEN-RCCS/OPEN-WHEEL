...
describe("remote host", ()=>{
  ...
  /**
   * Load the remote host settings page once before all tests to avoid the Chrome 146
   * renderer crash that occurs after ~12 page navigations in the same renderer process.
   */
  describe("remote host tests", { testIsolation: false }, ()=>{
    before(()=>{
      cy.openRemoteHostMenu();
    });

    afterEach(()=>{
      //Close the add new host dialog if still open, and navigate back if a test left the page
      ...
      //Remove the test host entry without navigating away
      cy.removeRemoteHostInPlace(LABEL);
    });
  ...
  試験確認内容：リモートホスト設定画面に遷移することを確認
    ...
    it("リモートホスト設定画面に遷移することを確認", ()=>{
      //before() opened the remote host dialog via navigation; verify the data table is visible.
  ...
  試験確認内容：リモートホスト設定画面に遷移することを確認
    ...
    it("構成要素の機能動作確認-ホーム画面が表示されていることを確認", ()=>{
      //Close the remote host dialog and verify the home screen is accessible
      cy.get("[data-cy=\"home-remote_host_close-btn\"]").click();
  ...
  試験確認内容：テキストが正しく入力されていることを確認
    ...
    it("構成要素の設定入力確認-テキスト入力-「Host work dir」テキストエリア-テキストが正しく入力されていることを確認", ()=>{
      ...
      cy.get("[data-cy=\"add_new_host-work_dir_label-text_field\"]").find("input")
        .should("have.value", HOST_WORK_DIR);
    });
  ...
  試験確認内容：テキストが正しく入力されていることを確認
    ...
    it("構成要素の設定入力確認-テキスト入力-「private key path」テキストエリア-テキストが正しく入力されていることを確認", ()=>{
      ...
      cy.get("[data-cy=\"add_new_host-private_key_path-text_field\"]").find("input")
        .should("have.value", PRIVATE_KEY_FILE);
    });
  ...
  試験確認内容：選択した値が正しく反映されていることを確認
    ...
    it("構成要素の設定入力確認-テキスト入力-「job scheduler」テキストエリア-選択した値が正しく反映されていることを確認", ()=>{
      ...
      cy.get("[data-cy=\"add_new_host-job_schedulers-select\"]").find("input")
  ...
  試験確認内容：選択した値が正しく反映されていることを確認
    ...
    it("構成要素の設定入力確認-テキスト入力-「shared host」セレクトボックス-選択した値が正しく反映されていることを確認", ()=>{
      ...
      cy.get("[data-cy=\"add_new_host-ok-btn\"]").should("not.be.disabled")
        .click();
      cy.get("[data-cy=\"remotehost-new_remote_host_setting-btn\"]").click();
      //Open the v-select dropdown then click the matching list item
      cy.get("[data-cy=\"add_new_host-shared_host-select\"]").click();
      cy.get(".v-list-item").contains(LABEL)
        .click();
  ...
  試験確認内容：テキストが正しく入力されていることを確認
    ...
    it("構成要素の設定入力確認-テキスト入力-「connection renewal interval (min.)」テキストエリア-テキストが正しく入力されていることを確認", ()=>{
      ...
      cy.get("[data-cy=\"add_new_host-connection_renewal-text_field\"]").find("input")
        .should("have.value", INTERVAL_MIN);
    });
  ...
  試験確認内容：テキストが正しく入力されていることを確認
    ...
    it("構成要素の設定入力確認-テキスト入力-「status check interval (sec.)」テキストエリア-テキストが正しく入力されていることを確認", ()=>{
      ...
      cy.get("[data-cy=\"add_new_host-status_check-text_field\"]").find("input")
        .should("have.value", STATUS_CHECK_SEC);
    });
  ...
  試験確認内容：テキストが正しく入力されていることを確認
    ...
    it("構成要素の設定入力確認-テキスト入力-「max number of status check error allowed」テキストエリア-テキストが正しく入力されていることを確認", ()=>{
      ...
      cy.get("[data-cy=\"add_new_host-max_number-text_field\"]").find("input")
        .should("have.value", HOST_MAX_NUMBER);
    });
  ...
  }); //end describe("remote host tests")

  /**
   * Verify that the remote host management dialog can also be opened
   * from the workflow screen via the navigation drawer.
   */
  describe("remote host dialog from workflow screen", ()=>{
    it("ワークフロー画面からリモートホスト設定ダイアログが開けることを確認", ()=>{
      cy.createAndOpenProject();
      cy.get("[data-cy=\"workflow-project_name-text\"]").should("be.visible");
      cy.get("[data-cy=\"tool_bar-navi-icon\"]").click();
      cy.get("[data-cy=\"navigation-manage_remote_host-btn\"]").click();
      cy.get("[data-cy=\"workflow-remote_host_management-title\"]").should("be.visible");
      cy.get("[data-cy=\"remotehost-items-data_table\"]").should("be.visible");
      cy.get("[data-cy=\"workflow-remote_host_close-btn\"]").click();
      cy.removeAllProjects();
    });
  }); //end describe("remote host dialog from workflow screen")
});
