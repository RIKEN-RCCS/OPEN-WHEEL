...
describe("home", ()=>{
  ...
  it("構成要素の機能動作確認-ボタン押下時の確認-BATCH MODE押下-複数プロジェクトが選択可能であることを確認", ()=>{
    ...
    cy.get("@projectCheckbox").eq(2)
      .check()
      .should("be.checked");
    cy.get("@projectCheckbox").eq(3)
      .check()
      .should("be.checked");
    cy.removeAllProjects();
  });
   ...
   */
  it.skip("構成要素の機能動作確認-ボタン押下時の確認-ハンバーガーボタン押下-ドロワーが表示されることを確認", ()=>{
  ...
  it("機能利用フローの確認-新規プロジェクトの作成-プロジェクトデータ作成場所選択-ディレクトリツリーでプロジェクトデータ作成場所が選択できることを確認", ()=>{
    ...
    cy.waitProjectList();
    cy.get("[data-cy=\"home-new-btn\"]").click();
    cy.get("[data-cy=\"home-project_list-data_table\"]", { timeout: 5000 })
      .should("contain.text", PROJECT_NAME);
    cy.get("[data-cy=\"home-file_browser-file_browser\"]")
      .contains(PROJECT_NAME + EXTENSION)
      .click();
    cy.get("[data-cy=\"home-file_browser-file_browser\"]")
  ...
  it("機能利用フローの確認-新規プロジェクトの作成-createボタン押下-プロジェクトが作成できていることを確認", ()=>{
    ...
    cy.get("[data-cy=\"home-project_description-btn\"]").contains(PROJECT_DESCRIPTION)
      .should("exist");
    cy.get("[data-cy=\"home-path-span\"]").contains(PROJECT_NAME + EXTENSION)
      .should("exist");
    cy.removeAllProjects();
  });
   ...
   */
  it.skip("機能利用フローの確認-プロジェクトを開く-プロジェクト名チェックボックス押下-複数プロジェクトを選択できないことを確認", ()=>{
    ...
    cy.get("[data-cy=\"home-project_list-data_table\"]").find("[type=\"checkbox\"]")
      .as("openCheckbox");
    cy.get("@openCheckbox").first()
      .check();
    cy.get("@openCheckbox").eq(1)
      .check();
    cy.get("@openCheckbox").first()
