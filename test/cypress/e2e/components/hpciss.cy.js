describe("components", ()=>{
  describe("hpciss", ()=>{
    ...
    const DEF_COMPONENT_HPCISS = "hpciss";
    const HPCISS_NAME_0 = "HPCI-SS0";
    const HPCISS_NAME_1 = "HPCI-SS1";
  ...
  試験確認内容：コンポーネントが接続されていることを確認
    ...
    it("ファイル転送設定の各パターンの確認-接続確認-コンポーネントが接続されていることを確認", ()=>{
      ...
      cy.createComponent(DEF_COMPONENT_HPCISS, HPCISS_NAME_1, 501, 600);
      cy.get("[data-cy=\"component_property-close-btn\"]").click(); //Close second component property panel
      cy.connectComponentMultiple(HPCISS_NAME_0, HPCISS_NAME_1); //コンポーネント同士を接続
      cy.checkConnectionLine(HPCISS_NAME_0, HPCISS_NAME_1); //作成したコンポーネントの座標を取得して接続線の座標と比較
    });
  ...
  試験確認内容：directory pathテキストボックスが表示されていることを確認
     */
    it("各コンポーネント特有のプロパティ確認-directory path表示確認-directory pathテキストボックスが表示されていることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_HPCISS, HPCISS_NAME_0, 501, 500);
  ...
  試験確認内容：directory pathが入力できることを確認
     */
    it("各コンポーネント特有のプロパティ確認-directory path入力確認-directory pathが入力できることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_HPCISS, HPCISS_NAME_0, 501, 500);
  ...
  試験確認内容：directory pathが反映されることを確認
     */
    it("各コンポーネント特有のプロパティ確認-directory path入力反映確認-directory pathが反映されることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_HPCISS, HPCISS_NAME_0, 501, 500);
      cy.get("[data-cy=\"component_property-directory_path-text_field\"]").type("test/test");
      cy.closeProperty();
      cy.clickComponentName(HPCISS_NAME_0);
