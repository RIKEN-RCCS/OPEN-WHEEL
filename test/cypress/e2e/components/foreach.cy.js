...
describe("components", ()=>{
  ...
  skip:issue#948
    ...
    it("構成要素の機能確認-clean component実行-最新の保存状態に戻っていることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_FOREACH, FOREACH_NAME_0, 501, 500);
      cy.closeProperty();
      cy.setComponentStateFinished(FOREACH_NAME_0);
      ...
      cy.clickComponentName(FOREACH_NAME_0);
      cy.get("[data-cy=\"component_property-name-text_field\"]").find("input")
        .should("have.value", FOREACH_NAME_0);
    });
  ...
  試験確認内容：インデックス値テキストボックスに入力した値が反映されていることを確認
     */
    it("プロパティ設定確認-インデックス値テキストボックス入力反映確認-インデックス値テキストボックスに入力した値が反映されていることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_FOREACH, FOREACH_NAME_0, 501, 500);
      cy.get("[data-cy=\"component_property-loop_set_foreach-panel_title\"]").click();
      cy.get("[data-cy=\"component_property-index_foreach-list_form\"]").find("input")
        .type(10);
  ...
  試験確認内容：number of instances to keepテキストボックスに入力した値が反映されていることを確認
     */
    it("プロパティ設定確認-keepテキストボックス入力反映確認-keepテキストボックスに入力した値が反映されていることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_FOREACH, FOREACH_NAME_0, 501, 500);
      cy.get("[data-cy=\"component_property-loop_set_foreach-panel_title\"]").click();
