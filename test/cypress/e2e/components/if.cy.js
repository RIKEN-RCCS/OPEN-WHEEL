describe("components", ()=>{
  ...
  試験確認内容：output files入力テキストエリアが表示されていることを確認
     */
    it("0output files表示-output files入力テキストエリアが表示されていることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_IF, IF_NAME_0, 501, 500);
  ...
  skip:issue#948
    ...
    it("構成要素の機能確認-clean component実行-最新の保存状態に戻っていることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_IF, IF_NAME_0, 501, 500);
      cy.closeProperty();
      cy.setComponentStateFinished(IF_NAME_0);
  ...
  試験確認内容：コンポーネントが接続されていることを確認
    ...
    it("ファイル転送設定の各パターンの確認-接続確認-コンポーネントが接続されていることを確認", ()=>{
      ...
      cy.createComponent(DEF_COMPONENT_IF, IF_NAME_1, 501, 600);
  ...
  試験確認内容：シンポリックリンクが作成されていることを確認
    ...
    it("ファイル転送設定の各パターンの確認-シンポリックリンク確認（outputFileが通常、inputFileが「/」で終わらない文字列）-シンポリックリンクが作成されていることを確認", ()=>{
      ...
      cy.selectValueFromDropdownList(targetDropBoxCy, 3, "test-b"); //scritファイル選択
      cy.closeProperty();
      cy.wait(100); //select script needs some wait time
      cy.connectComponentMultiple(IF_NAME_0, IF_NAME_1); //コンポーネント同士を接続
      cy.clickComponentName(IF_NAME_1);
      ...
      cy.get("[data-cy=\"list_form_property-text_field\"]").find("input")
        .clear()
        .type("if1.sh{enter}"); //inputFileの値を変更
      cy.closeProperty();
      cy.get("[data-cy=\"workflow-play-btn\"]").click(); //実行する
      cy.checkProjectStatus("finished");
      cy.clickComponentName(IF_NAME_1);
