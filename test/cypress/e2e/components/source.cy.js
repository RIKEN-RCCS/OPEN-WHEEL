describe("components", ()=>{
  ...
  skip:issue#948
    ...
    it("構成要素の機能確認-clean component実行-最新の保存状態に戻っていることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_SOURCE, SOURCE_NAME_0, 501, 500);
      cy.closeProperty();
      cy.setComponentStateFinished(SOURCE_NAME_0);
      ...
      cy.clickComponentName(SOURCE_NAME_0);
      cy.get("[data-cy=\"component_property-name-text_field\"]").find("input")
        .should("have.value", SOURCE_NAME_0);
    });
  ...
  試験確認内容：ディレクトリが単体表示されることを確認
     */
    it("ファイル操作エリア-ディレクトリ複数表示-ディレクトリが単体表示されることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_SOURCE, SOURCE_NAME_0, 501, 500);
  ...
  試験確認内容：ファイルが単体表示されることを確認
     */
    it("ファイル操作エリア-ファイル複数表示-ファイルが単体表示されることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_SOURCE, SOURCE_NAME_0, 501, 500);
  ...
  試験確認内容：upload on demandスイッチボタン表示確認が表示されていることを確認
     */
    it("各コンポーネント特有のプロパティ確認-upload on demandスイッチボタン表示確認-upload on demandスイッチボタン表示確認が表示されていることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_SOURCE, SOURCE_NAME_0, 501, 500);
  ...
  試験確認内容：source file nameが入力できることを確認
     */
    it("各コンポーネント特有のプロパティ確認-source file name入力確認-source file nameが入力できることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_SOURCE, SOURCE_NAME_0, 501, 500);
      cy.get("[data-cy=\"component_property-upload_setting-panel_title\"]").click();
      cy.createDirOrFile(TYPE_FILE, "test-a", true);
      let targetDropBoxCy = "[data-cy=\"component_property-source_file_name-autocomplete\"]";
  ...
  試験確認内容：source file nameが反映されることを確認
     */
    it("各コンポーネント特有のプロパティ確認-source file name入力反映確認-source file nameが反映されることを確認", ()=>{
      cy.createComponent(DEF_COMPONENT_SOURCE, SOURCE_NAME_0, 501, 500);
      cy.get("[data-cy=\"component_property-upload_setting-panel_title\"]").click();
      cy.createDirOrFile(TYPE_FILE, "test-a", true);
      let targetDropBoxCy = "[data-cy=\"component_property-source_file_name-autocomplete\"]";
