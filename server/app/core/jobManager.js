...
export function registerJob(hostinfo, task) {
  return new Promise((resolve, reject)=>{
    ...
    result.event.on("finished", async (request)=>{
      ...
      let rt = await _internal.getStatusCode(JS, task, hook.rt, hook.output);
      ...
      if (rt === -2 && !reEmpty.test(hook.output)) {
        _internal.getLogger(task.projectRootDir).trace(`${requestName} after cmd output was ambiguous, rechecking once`);
        await recheck();
        rt = await _internal.getStatusCode(JS, task, hook.rt, hook.output);
      }
