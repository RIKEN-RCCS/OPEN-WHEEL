...
For now, change the drop-down list to __list__ and enter three values: __foo____bar____baz__.
...
WHEEL uploads a file at run time as input data for the workflow.
...
By writing __\* .extension __, you can specify all files with the same extension or omit the file name. (For example, for a JPEG file, write __\*.jpg__)  
...
About setting __directory path __  
...
__ Remote host settings when using the bulk job function __  
...
__ Remote host settings when using the step job function __  
