...
In WHEEL, the execution unit of a workflow is called a project.
In a project, you create one or more task components that actually run the program.
...
On the workflow screen, you can use the palette on the left side of the screen to insert the __ component __ that is a part of the workflow.
...
All components, including the Task component, can do this by setting the
For the first time, it functions as a workflow part.
...
__ About the revert project button __  
...
When you click the __host__ drop-down list and, the list of remote hosts configured in Remote Host Configuration ({{site.baseurl}}/how_to_boot/# Remote Host Configuration) is displayed.
...
__ About Preparing to Run Tasks Through a Batch System __   
...
From Files at the bottom of the Properties window, select 3. Task Component Property Settings (#3- Task Component Property Settings)
Select the script you created in and open it in a text editor.
...
__ How to discard file edits __  
...
{% capture notice-save %}
__ How to save only some files __  
...
1. Save (Or use the __save__ button to save the required files individually.) with the __SAVE ALL FILES__ button.
{% endcapture %}
