...
画面上部にある __stop and cleanup project__ ボタンをクリックして、実行開始前の状態に戻してください。
...
__host__ ドロップダウンリストとをクリックすると、[初期設定]({{ site.baseurl }}/how_to_boot/#初期設定)で設定されたremotehostの一覧が表示されるので、
...
出力内容を確認したら、__stop and cleanup project__ ボタンをクリックしてプロジェクトを初期化してください。
...
バッチシステム経由で実行する場合は、事前に[バッチシステム経由でのタスク実行]({{ site.baseurl }}/how_to_boot/#バッチシステム経由でのタスク実行)に従いリモートホスト設定が実施されている必要があります。
{: .notice}
