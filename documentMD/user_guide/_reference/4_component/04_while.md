...
インスタンスディレクトリ名が既存のコンポーネントやファイル・ディレクトリと衝突した場合の区切り文字のエスカレーション（`_` → `__` → `___`）についても、Forコンポーネントと同様です。詳しくは[Forコンポーネント実行時の挙動]({{ site.baseurl }}/reference/4_component/03_For.html#forコンポーネント実行時の挙動)を参照してください。
