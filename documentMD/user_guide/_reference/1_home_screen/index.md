...
githubなどのリポジトリから直接インポートする場合は
アーカイブファイルを指定する代わりに(4) `import from git repository` ボタンをクリックして
URLを入力し、__OK__ボタンをクリックしてください。

なお、認証が必要なgitリポジトリからのインポートには対応していません。

__ディレクトリ表示について__
本画面では、Openボタンを押下した時と同様にベースディレクトリ配下の
ディレクトリ構成が表示されますが、ベースディレクトリ直下にプロジェクトをインポートするために
カレントディレクトリを意味する `./` も表示されています。
`./` をクリックして選択することはできますが、他のディレクトリとは異なり開いてもこの下にディレクトリツリーは表示されません。
{: .notice--info}
