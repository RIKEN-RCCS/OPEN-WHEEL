...
1. ターミナルを起動し、以下のコマンドを入力します。
    ...
    |オプション|説明|
    |-----|-----|
    |-v|Dockerが選択したホストディレクトリにバインドします。コンテナ内のディレクトリのみを指定できます。|
    |-p|ポート番号を割り当てます。一般的な指定方法は以下のとおりです。<br/>　-p <ホストマシンのポート>:<コンテナのポート><br/>　　ホストマシンのポート：ホストマシンの特定のポート番号を指定します。<br/>　　コンテナのポート：コンテナ内の待ち受けるポート番号を指定します。<br/>複数のポートをマッピングする場合は、スペースで区切って指定します。|
