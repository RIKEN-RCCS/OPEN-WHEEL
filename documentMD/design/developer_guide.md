...
1. **全ての変更は** `client/`, `server/`, `common/` ディレクトリ内で行う
2. `client/` または `common/` を変更した場合、必ずクライアントコードをビルドする
3. コード変更後は必ずlintを実行する
4. **新しい関数を追加した場合** は JSDoc コメントを付ける
5. **`server/` 以下に新関数を追加した場合** はユニットテストを追加する
6. **UIに影響する変更** はCypressコンポーネントテストを追加する
7. Gitへのコミット・リバート等は明示的に指示された場合のみ行う
8. `server/app/db/version.json` は変更しない（GitHub Workflowが自動更新）
9. ユニットテストでの条件付きスキップ（`skip()`）は使用しない
...
- **`db/version.json` は変更しない** — CI/CDが自動更新する
- **クライアント変更後のビルド忘れ** — 変更が反映されない
- **環境変数を `process.env` から直接読む** — 代わりに `db.js` からインポートする
- **ログに `console.log` を残す** — コミット前に必ず削除する
- **テストで `.only` を残す** — 他のテストが実行されなくなる
- **JSDoc なしで新関数を追加** — レビューで指摘される
