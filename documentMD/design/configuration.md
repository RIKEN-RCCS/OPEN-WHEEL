...
### デバッグ変数（環境変数専用・`server.json` にマッピングされない）

| 環境変数 | 説明 |
|---------|------|
| `WHEEL_DEBUG_METADATA_JSON` | HPCI-SS メタデータJSONを指定パスに書き出す。固定パスへ毎回上書き（詳細・上書き挙動は `gfarm.md` の「デバッグ用メタデータ書き出し」節を参照） |
| `WHEEL_DEBUG_METADATA_XML` | 同上のXML版 |
